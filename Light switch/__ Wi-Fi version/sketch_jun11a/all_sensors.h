...
uint32_t sTimer1_buf;
uint32_t sTimer2_buf;
float sTemperatureS_buf;
bool sRelay_buf = false;
bool sBarrel_buf = false;
  ...
  public: 
    ...
    void run() {
      ...
      if ( millis() - timerSensor >  INTERVAL_SENSOR_READIGS  || flag ) {
        ...
        #ifdef SET_RELAY
          sRelay.read( ( sRelay_buf ) ? 1 : 0, true );
        #endif

        #ifdef SET_BARREL
          sBarrel.read( ( ! sBarrel_buf ) ? 1 : 0, true );
        #endif

        #ifdef SET_TIMER_1
          sTimer1.read(sTimer1_buf);
        #endif

        #ifdef SET_TIMER_2
          sTimer2.read(sTimer2_buf);
        #endif
